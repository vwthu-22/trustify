...
export default function MyReviewPage() {
  ...
  // Debug: Log reviews to check likes field
  useEffect(() => {
    if (myReviews.length > 0) {
      console.log('My Reviews Data:', myReviews);
      console.log('First review likes:', myReviews[0]);
      console.log('Total likes:', myReviews
        .filter(r => r.status === 'APPROVED' || !r.status)
        .reduce((total, review) => {
          console.log(`Review ${review.id} likes:`, (review as any).likes);
          return total + ((review as any).likes || 0);
        }, 0)
      );
    }
  }, [myReviews]);
